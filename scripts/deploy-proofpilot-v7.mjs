...
console.log(`Submitted ProofPilot v7 deployment: ${explorer}/tx/${deployTransactionHash}`)
...
if (receipt.txExecutionResultName !== 'FINISHED_WITH_RETURN') {
  throw new Error(`v7 deployment did not execute successfully: ${receipt.txExecutionResultName ?? 'UNKNOWN'}. Do not configure the frontend or redeploy blindly.`)
}
...
console.log(JSON.stringify({
  ...
  next: campaigns
    ? 'Wait for FINALIZED in GenExplorer. Then update src/lib/deployment.ts, README, and Vercel with this final address and run the v7 end-to-end checklist.'
    : `Do not redeploy. Recheck ${contractAddress} after the public GenVM RPC syncs.`,
}, null, 2))
