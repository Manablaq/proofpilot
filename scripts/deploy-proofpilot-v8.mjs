...
console.log(`Submitted ProofPilot V8 deployment: ${explorer}/tx/${deployTransactionHash}`)
...
if (receipt.txExecutionResultName !== 'FINISHED_WITH_RETURN') {
  throw new Error(`V8 deployment did not execute successfully: ${receipt.txExecutionResultName ?? 'UNKNOWN'}. Do not configure the frontend or redeploy blindly.`)
}
...
console.log(JSON.stringify({
  release: 'V8', deployTransactionHash, contractAddress, listCampaigns: campaigns,
  ...
  next: campaigns ? 'Wait for FINALIZED in GenExplorer. Then update src/lib/deployment.ts and verify the Vercel frontend against this final address.' : `Do not redeploy. Recheck ${contractAddress} after public RPC syncs.`,
}, null, 2))
